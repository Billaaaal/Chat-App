...
//import the dialog module from the react native dialogs package
...
function storeHighScore(user, message) {
  ...
  const reference = ref(db, 'messages/'  + Date.now());   //ref(db, 'messages/'  +"("+Date.now()+")" + message);// //Math.floor(Math.random() * 111111111));
  set(reference, {
    message: message,
    user: user,
  });
...
export default function App() {
  ...
  const[TextFromDialog, setTextFromDialog] = useState([]);
  ...
  return (
    ...
    <Dialog.Container visible={isDialogVisible}>
        <Dialog.Description>Enter a username</Dialog.Description>
        <Dialog.Input
        //label="Password"
        //placeholder="Enter your password"
          onChangeText={(text) => setTextFromDialog(text)}/>
          <Dialog.Button label="Submit" onPress={() => {setisDialogVisible(false)}} />
      ...
      style={{top:40}}
        ...
        (<View key={index} style={{width:300, height:40, borderRadius:10 , backgroundColor:"#dcfff9", marginBottom:20, left:20}}><Text style={{left:20, top:10, color:"#f"}}>{single_message}</Text></View>
        ))}
    ...
    <TextInput style={{width:380, height:50, backgroundColor:"#EFEFEF", borderRadius:15, marginBottom:5, left:5, marginTop:50}}   onChangeText={(text) => setTextFromInput(text)} onSubmitEditing={()=>{storeHighScore(TextFromDialog, TextFromInput)}}></TextInput>
